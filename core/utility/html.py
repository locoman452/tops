...
Classes to allow (X)HTML files to be generated via python declarations.
"""

## @package tops.core.utility.html
# Generates (X)HTML files via python declarations
#
...
class Entity(Text):
	...
	allowed = [ # http://en.wikipedia.org/wiki/List_of_XML_and_HTML_character_entity_references
		'quot', 'amp', 'apos', 'lt', 'gt', 'nbsp', 'iexcl', 'cent', 'pound', 'curren', 'yen', 'brvbar',
        ...
        'uuml', 'yacute', 'thorn', 'yuml', 'OElig', 'oelig', 'Scaron', 'scaron', 'Yuml', 'fnof', 'circ',
        ...
        'chi', 'psi', 'omega', 'thetasym', 'upsih', 'piv', 'ensp', 'emsp', 'thinsp', 'zwnj', 'zwj', 'lrm',
        'rlm', 'ndash', 'mdash', 'lsquo', 'rsquo', 'sbquo', 'ldquo', 'rdquo', 'bdquo', 'dagger', 'Dagger',
        'bull', 'hellip', 'permil', 'prime', 'Prime', 'lsaquo', 'rsaquo', 'oline', 'frasl', 'euro', 'image',
        'weierp', 'real', 'trade', 'alefsym', 'larr', 'uarr', 'rarr', 'darr', 'harr', 'crarr', 'lArr',
...
class Element(object):
	...
	def setAttribute(self,key,value):
		...
		if key in self.attributes:
			raise HTMLDocumentError('attribute has already been set: %s = %s' %
		...
		if key == 'id' and not self.root is None:
	...
	def setRoot(self,root):
		...
		"""
		self.root = root
		if 'id' in self.attributes and not root is None:
	...
	def append(self,child):
		...
		if isinstance(child,Element) and not self.root is None:
...
# Define the allowed HTML4 block-level elements according to http://htmlhelp.com/reference/html40/block.html
...
# Define the allowed HTML4 inline elements according to http://htmlhelp.com/reference/html40/inline.html
# Python symbols are capitalized to avoid conflicts with builtin names (like 'object')
for tag in [
...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en" lang="en">\n'''
			+ self.head.__str__() + self.body.__str__() + '\n</html>')
